...
class PropertiesSuite extends FunSuite { // tests.Suite
  ...
  test("list") {
    ...
    val result2 =
      """-- listing properties --
        |name=alice
        |p0000000000111111111122222222223333333333=40
      """.stripMargin
  ...
  test("load(InputStream)") {
    ...
    assert(null == prop.getProperty("commented.key"))
  ...
  test("load(InputStream) handle whitespace") {
    ...
    assertEquals("b", prop.getProperty("a"))

    prop = new java.util.Properties()
    prop.load(new ByteArrayInputStream(" a b".getBytes()))
    assertEquals("b", prop.get("a").asInstanceOf[String])
  }
  ...
  test("load(InputStream) handle special chars") {
    ...
    assertEquals("\u008d\u00d3", prop.getProperty("a"))
  ...
  test("check comment formatted correctly") {
    ...
    assert(true == out1.toString().startsWith(commentsWithoutDate))
  }
  ...
  test("check properties formatted correctly") {
    ...
    //println(res)
    assert(true == out1.toString().endsWith(res))
  }
